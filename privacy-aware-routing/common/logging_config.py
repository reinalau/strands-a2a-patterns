"""Shared logging configuration used by both processes of the use case.

Each process (orchestrator and remote agent) calls `setup_logging(...)` with its
own file name. Logs go simultaneously to:

- the console (to follow the A2A delegation moment in real time), and
